...
useful_count_labels = {0: "0", 1: "1~2", 2: "3~4", 3: "5~6", 4: "7~9", 5: "10~"}
...
def save_vectorized_review_csv(
    train_df: DataFrame, test_df: DataFrame, model: Doc2Vec
) -> None:
    """
    ...
    os.makedirs(f"{current_path}/binaries/d2v/no4", exist_ok=True)
    np.savez(
        f"{current_path}/binaries/d2v/no4/vectorized_train.npz",
        label=vectorized_train_df["label"].values,
        text=vectorized_train_df["text"].values,
    )
    np.savez(
        f"{current_path}/binaries/d2v/no4/vectorized_test.npz",
...
def main():
    ...
    train_df = pd.read_csv(
        f"{current_path}/../csv/text_classification/all/train.csv", index_col=0
    )
    test_df = pd.read_csv(
        f"{current_path}/../csv/text_classification/all/test.csv", index_col=0
    )
    ...
    model = Doc2Vec(documents=trainings, dm=1, vector_size=200, min_count=0, epochs=20)
    os.makedirs(f"{current_path}/models/all/d2v/rf", exist_ok=True)
    model.save(f"{current_path}/models/all/d2v/rf/no4.model")

    # 　学習用・テスト用データの`text`カラムをベクトル化したものに置換
    model = Doc2Vec.load(f"{current_path}/models/all/d2v/rf/no4.model")
